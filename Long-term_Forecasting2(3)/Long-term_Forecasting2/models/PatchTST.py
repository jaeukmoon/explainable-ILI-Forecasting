...
class PatchTST(nn.Module):
    ...
    def __init__(self, configs, device):
                ...
                EncoderLayer(
                    AttentionLayer(
                        FullAttention(False, self.factor, attention_dropout=configs.dropout,
                                      ...
                                      configs=configs),
                                      configs.d_model, configs.n_heads, d_keys=2, d_values=2),
                    configs.d_model,
                    configs.d_ff,
        ...
        self.proj = nn.Linear(8, configs.pred_len, bias=True)
        self.padding_patch_layer = nn.ReplicationPad1d((0, self.stride)) # 패딩 추가
        self.cnt = 0
