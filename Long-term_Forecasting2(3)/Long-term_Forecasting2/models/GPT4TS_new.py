...
class GPT4TS(nn.Module):
    ...
    def __init__(self, configs, device):
        ...
        for layer in (self.llmmodel, self.in_layer, self.out_layer,self.text_layer):
    ...
    def forward(self, x, itr):
        ...
        if self.multimodal == 1:
            text_emb = np.loadtxt('./dataset/illness/text_emb/'+self.textmodel+'text_emb_label_0617_'+str(self.pred_len)+'.txt')
            # print(text_emb)
            text_emb = np.tile(text_emb, (B, 1, 1))
            text_emb_tensor = torch.tensor(text_emb, dtype=torch.float32, device=outputs.device)
            text_output = self.text_layer(text_emb_tensor)


            outputs = torch.cat((outputs,text_output), axis = 1)
