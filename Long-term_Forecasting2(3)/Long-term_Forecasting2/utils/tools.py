...
def test(model, test_data, test_loader, args, device, itr):
    ...
    with torch.no_grad():
        for i, (batch_x, batch_y) in tqdm(enumerate(test_loader)):
            ...
            batch_y = batch_y.float() # torch.Size([16, 42, 1])

            outputs = model(batch_x[:, -args.seq_len:, :], itr)

            # encoder - decoder
            outputs = outputs[:, -1:, :]
            batch_y = batch_y[:, -1:, :].to(device) # torch.Size([16, 24, 1])
    ...
    rmse, pcc = evaluation(preds, trues)
    ...
    print('L:{:.4f}, rmse:{:.4f}, pcc:{:.4f}'.format(args.pred_len, rmse, pcc))

    return rmse, pcc

def test_vis(model, test_data, test_loader, args, setting, device):
    preds = []
    trues = []
    # mases = []
    # setting = '{}_multimodal{}_sl{}_ll{}_pl{}_dm{}_nh{}_el{}_gl{}_df{}_eb{}'.format(args.model_id, args.multimodal, args.seq_len, args.label_len, args.pred_len,
    ...
    csv_file = pd.concat([preds, trues],axis = 1)
    csv_file.to_csv(setting+'results.csv')

    plt.figure(figsize=(10, 5))
    plt.plot(trues, label='Actual Data')
