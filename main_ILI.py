...
parser.add_argument("--output_path", type=str, default="./saved_models/Llama-3.2-3B-Instruct-CoT_date_0115")
...
parser.add_argument("--prompt_template", type=str, default="ILI_PREDICT_INSTRUCTION_PROMPT_INSTRUCT_CoT_date_0115")
parser.add_argument("--explanation_prompt", type=str, default="ILI_EXPLANATION_GENERATION_GPT_0110")
...
parser.add_argument('--features', type=str, default='SS')
parser.add_argument('--freq', type=int, default=0)
parser.add_argument('--target', type=str, default='Region 3')
...
def train():
    ...
    for ii in range(args.itr):
        ...
        data_news_exp_pastpop = data_news[data_news['start_past_X_date'].notna()].reset_index(drop = True)
        ...
        exp_list, past_exp_list = exp_agent.explanation_files_generation(data=data_news_exp_pastpop)
        data_news_exp_pastpop['explanations'] = exp_list
        data_news_exp_pastpop['past_explanations'] = past_exp_list
...
def test_live_new():
    ...
    for i, row in data_news_test_pastpop.iterrows():
        ...
        prompt = prompt_template.format(start_X_date = row["start_X_date"],
                                ...
                                past_region_number = row["past_region_number"],
...
def test_live():
    ...
    exp_filename = args.exp_llm
    exp_file_path = os.path.join('./explanations/', exp_filename)
    exp_list, past_exp_list = exp_agent.explanation_files_generation(data=data_news_test_pastpop)
    ...
    def generate_response(row):
        ...
        predicted_Y = result.split('[OUTPUT]')[-1] if '[OUTPUT]' in result else ""
        ILI_occur = predicted_Y.split('Explanation:')[0].split('Future ILI occurrences:')[-1]
